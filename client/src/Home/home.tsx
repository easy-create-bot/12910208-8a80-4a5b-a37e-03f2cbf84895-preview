    ...
    return <section>
        <div className="flex flex-col items-center space-y-8 px-4 ">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 w-[100%] mx-auto gap-4 justify-items-center">
                {subscriptions?.map(({ id: subscriptionId, tier, points, price }, index) => {
                    const isThirdCard = index === 2;
                    return (
                        <div key={subscriptionId} className={isThirdCard ? "lg:col-span-2 xl:col-span-1" : ""}>
                            <SubscriptionCard subscriptionId={subscriptionId} tier={tier} points={points} price={price} />
                        </div>
                    )
                })}
            </div>
...
function SubscriptionCard({ subscriptionId, tier, points, price }: { subscriptionId: number, tier: string, points: string[], price: number }) {
    const { id } = useAuth()

    const handleSubscriptionCheckout = async (subscriptionId: number) => {
        try {
    ...
    return <Card className="w-[300px] md:w-[400px] h-[450px] mx-auto xl:mx-0 border-0 shadow-[#3c3b3a] card shadow-sm p-0 rounded-t-xl">
        <CardContent className='px-0'>
            ...
            <div className="flex flex-col items-center mt-6">
                ...
                <Button onClick={() => handleSubscriptionCheckout(subscriptionId)} className="hover:bg-[#1b0f27] cursor-pointer w-fit select-trigger mt-12 bg-[#1b0f27] text-white text-lg md:text-2xl font-header">
                    Choose {tier}
                </Button>
            </div>
        </CardContent>
